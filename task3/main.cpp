  ...
  for(unsigned int idim=0;idim<ndim;++idim){
    for(unsigned int jdim=0;jdim<ndim;++jdim) {
      ...
      // ddC[ino][jno][ino][jno] means differentiation of C w.r.t. p[ino][idim] and then p[jno][jdim]
        int num = (idim == jdim);
        ddC[0][0][idim][jdim] = num / len - u01[idim] * u01[jdim] / len;
        ddC[0][1][idim][jdim] = -1 * num / len + u01[idim] * u01[jdim] / len;
        ddC[1][0][idim][jdim] = -1 * num / len + u01[idim] * u01[jdim] / len;
        ddC[1][1][idim][jdim] = num / len - u01[idim] * u01[jdim] / len;
    }
  }
  //
